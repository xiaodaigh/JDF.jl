...
column_loader(t::Type{T}, io, metadata) where {T} =
    begin
...
column_loader!(buffer, ::Type{T}, io, metadata) where {T} =
    begin
        readbytes!(io, buffer, metadata.len)
        return Blosc.decompress(T, buffer)
    end

compress_then_write(::Type{Bool}, b, io) =
    begin
        b8 = UInt8.(b)
...
column_loader(T::Type{Bool}, io, metadata) =
    begin
...
column_loader!(buffer, T::Type{Bool}, io, metadata) =
    begin
# Bool are saved as UInt8
        read!(io, buffer)
...
compress_then_write(::Type{T}, b, io) where {T>:Missing} =
    begin
...
column_loader!(buffer, ::Type{Union{Missing,T}}, io, metadata) where {T} =
    begin
# read the content
...
compress_then_write(_, b::StringVector{T}, io) where {T} =
    compress_then_write(b, io)

compress_then_write(b::StringVector{T}, io) where {T} =
    begin
...
column_loader(::Type{StringVector{T}}, io, metadata) where {T} =
    begin
# uncompress
        args = Vector[]
...
# perform a RLE
compress_then_write(::Type{String}, b::Array{String}, io) =
    begin
...
"""
column_loader!(_, ::Type{String}, io, metadata) =
    begin
